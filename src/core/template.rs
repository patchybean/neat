...
use crate::utils::metadata::{ImageMetadata, AudioMetadata, is_exif_supported, is_audio_supported};
...
impl TemplateEngine {
    ...
    pub fn from_file(file: &FileInfo, classifier: &Classifier) -> Self {
        ...
        variables.insert("extension".to_string(), 
            file.extension.clone().unwrap_or_else(|| "unknown".to_string()));
        variables.insert("ext".to_string(), 
            file.extension.clone().unwrap_or_else(|| "unknown".to_string()));
        
        // File size
        variables.insert("size".to_string(), file.size.to_string());
        variables.insert("size_kb".to_string(), (file.size / 1024).to_string());
        variables.insert("size_mb".to_string(), (file.size / (1024 * 1024)).to_string());
...
mod tests {
    ...
    fn test_preset_templates() {
        assert_eq!(get_preset_template("by-type"), Some("{category}/{filename}"));
        assert_eq!(get_preset_template("by-date"), Some("{year}/{month}/{filename}"));
