...
pub fn run(action: QuickAction) -> Result<()> {
    match action {
        ...
        QuickAction::Cleanup { days, trash, dry_run } => {
            let path = dirs::download_dir()
                .context("Could not find Downloads directory")?;
...
fn cleanup_old_files(path: &PathBuf, days: u32, use_trash: bool, execute: bool) -> Result<()> {
    ...
    println!("{} Found {} files older than {} days:", "→".yellow(), files.len(), days);
    ...
    if execute {
        ...
        println!("{} {} {} files? (this action is not undoable)", "⚠".yellow(), action, files.len());
